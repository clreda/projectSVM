...
L'hyperplan auquel $\omege$ est normal sépare l'espace dans deux parties qui representent deux classes. Pour obtenir un résultat robuste, on souhaite que la marge (la somme des distances entre l'hyperplan et les points les plus proches) soit les plus grands possibles. En effet, si la marge est pétit, cela signifie que la probabilité d'erreur quant à la prédiction de la classe d'un point proche de ces droites sera importante.\\
...
La marge $\gamma$ se calcule de la façon suivante : soient $u, v$ deux points plus proches au l'hyperplan tels que $f(v) = 1$ et $f(u) = -1$. Alors :
